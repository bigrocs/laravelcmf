...
if (! function_exists('getActiveNav')) {
     ...
     * @return [type]                        [description]
     */
    function getActiveNav($nav)
    {
        $routeName = routeName();
        foreach ($nav as &$navValue) {
            // array_dot 函数使用”.”号将将多维数组转化为一维数组：
            $array_dot = array_dot($navValue);
            foreach ($array_dot as $key=>$value) {
                // str_contains 函数判断是否包含
                if (str_contains($routeName,$value)) {
                    $navValue['active'] = true;
                }
            }
            if (isset($navValue['subNav'])) 
            {
                $navValue['subNav'] = getActiveNav($navValue['subNav']);
            }
        }
        return $nav;
    }
